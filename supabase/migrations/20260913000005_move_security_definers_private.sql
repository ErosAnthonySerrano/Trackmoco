-- Keep exposed public functions as invoker-only wrappers.
-- Privileged implementations live in a non-API schema.

CREATE SCHEMA IF NOT EXISTS private;

REVOKE ALL ON SCHEMA private FROM PUBLIC;
GRANT USAGE ON SCHEMA private TO authenticated;

CREATE OR REPLACE FUNCTION private.is_installment_member(
  p_installment_id uuid,
  p_user_id uuid DEFAULT auth.uid()
) RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT p_user_id = auth.uid()
    AND EXISTS (
      SELECT 1
      FROM public.installment_members im
      WHERE im.installment_id = p_installment_id
        AND im.user_id = auth.uid()
    );
$$;

CREATE OR REPLACE FUNCTION private.has_installment_role(
  p_installment_id uuid,
  p_roles public.member_role[],
  p_user_id uuid DEFAULT auth.uid()
) RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT p_user_id = auth.uid()
    AND EXISTS (
      SELECT 1
      FROM public.installment_members im
      WHERE im.installment_id = p_installment_id
        AND im.user_id = auth.uid()
        AND im.role = ANY (p_roles)
    );
$$;

CREATE OR REPLACE FUNCTION public.is_installment_member(
  p_installment_id uuid,
  p_user_id uuid DEFAULT auth.uid()
) RETURNS boolean
LANGUAGE sql
STABLE
SECURITY INVOKER
SET search_path = public, private
AS $$
  SELECT private.is_installment_member(p_installment_id, p_user_id);
$$;

CREATE OR REPLACE FUNCTION public.has_installment_role(
  p_installment_id uuid,
  p_roles public.member_role[],
  p_user_id uuid DEFAULT auth.uid()
) RETURNS boolean
LANGUAGE sql
STABLE
SECURITY INVOKER
SET search_path = public, private
AS $$
  SELECT private.has_installment_role(p_installment_id, p_roles, p_user_id);
$$;

REVOKE ALL ON FUNCTION private.is_installment_member(uuid, uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION private.has_installment_role(uuid, public.member_role[], uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.is_installment_member(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION private.has_installment_role(uuid, public.member_role[], uuid) TO authenticated;

REVOKE ALL ON FUNCTION public.is_installment_member(uuid, uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.has_installment_role(uuid, public.member_role[], uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.is_installment_member(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.has_installment_role(uuid, public.member_role[], uuid) TO authenticated;

CREATE OR REPLACE FUNCTION private.get_dashboard_summary(p_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_total_owed numeric;
  v_total_paid numeric;
  v_due_week_count int;
  v_due_week_amount numeric;
  v_overdue_count int;
  v_upcoming_count int;
  v_upcoming jsonb;
BEGIN
  IF p_user_id IS NULL OR p_user_id <> auth.uid() THEN
    RAISE EXCEPTION 'Not authorized';
  END IF;

  SELECT COALESCE(SUM(ii.amount), 0) INTO v_total_owed
  FROM public.installment_members im
  JOIN public.installment_items ii ON ii.installment_id = im.installment_id
  WHERE im.user_id = p_user_id AND ii.status = 'unpaid';

  SELECT COALESCE(SUM(ii.amount), 0) INTO v_total_paid
  FROM public.installment_members im
  JOIN public.installment_items ii ON ii.installment_id = im.installment_id
  WHERE im.user_id = p_user_id AND ii.status = 'paid';

  SELECT COUNT(*), COALESCE(SUM(ii.amount), 0)
  INTO v_due_week_count, v_due_week_amount
  FROM public.installment_members im
  JOIN public.installment_items ii ON ii.installment_id = im.installment_id
  WHERE im.user_id = p_user_id
    AND ii.status = 'unpaid'
    AND ii.due_date >= CURRENT_DATE
    AND ii.due_date <= (CURRENT_DATE + INTERVAL '7 days')::date;

  SELECT COUNT(*) INTO v_overdue_count
  FROM public.installment_members im
  JOIN public.installment_items ii ON ii.installment_id = im.installment_id
  WHERE im.user_id = p_user_id AND ii.status = 'unpaid' AND ii.due_date < CURRENT_DATE;

  SELECT COUNT(*) INTO v_upcoming_count
  FROM public.installment_members im
  JOIN public.installment_items ii ON ii.installment_id = im.installment_id
  WHERE im.user_id = p_user_id AND ii.status = 'unpaid';

  SELECT COALESCE(jsonb_agg(u ORDER BY u.due_date ASC), '[]'::jsonb) INTO v_upcoming
  FROM (
    SELECT
      i.id AS installment_id,
      i.title AS installment_title,
      ii.id AS item_id,
      ii.label AS item_label,
      ii.due_date,
      ii.amount
    FROM public.installment_members im
    JOIN public.installments i ON i.id = im.installment_id
    JOIN public.installment_items ii ON ii.installment_id = i.id
    WHERE im.user_id = p_user_id AND ii.status = 'unpaid'
    ORDER BY ii.due_date ASC
    LIMIT 5
  ) u;

  RETURN jsonb_build_object(
    'total_owed', v_total_owed,
    'total_paid', v_total_paid,
    'due_this_week', jsonb_build_object('count', v_due_week_count, 'amount', v_due_week_amount),
    'overdue_count', v_overdue_count,
    'upcoming_count', v_upcoming_count,
    'upcoming', v_upcoming
  );
END;
$$;

CREATE OR REPLACE FUNCTION public.get_dashboard_summary(p_user_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY INVOKER
SET search_path = public, private
AS $$
  SELECT private.get_dashboard_summary(p_user_id);
$$;

REVOKE ALL ON FUNCTION private.get_dashboard_summary(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.get_dashboard_summary(uuid) TO authenticated;
REVOKE ALL ON FUNCTION public.get_dashboard_summary(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_dashboard_summary(uuid) TO authenticated;
